...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    return self.isEditing == NO; //Only recognize gesture if cell is not in edit mode
}
...
//Initialize static vars(shared across instances) in load which is called once per class
+ (void)load {
...
- (NSAttributedString *)usernameAndCaptionString {
    ...
    NSMutableAttributedString *mutableUsernameString = [[NSMutableAttributedString alloc] initWithString:baseString attributes:@{NSFontAttributeName : [lightFont fontWithSize:usernameFontSize], NSParagraphStyleAttributeName: paragraphStyle }];
    
    NSRange usernameRange = [baseString rangeOfString:self.mediaItem.user.userName];
    [mutableUsernameString addAttribute:NSFontAttributeName value:[boldFont fontWithSize:usernameFontSize] range:usernameRange];
    
    [mutableUsernameString addAttribute:NSForegroundColorAttributeName value:linkColor range:usernameRange];
...
- (NSAttributedString *)commentString {
    ...
    for (BLCComment *comment in self.mediaItem.comments) {
        //Make string "username comment text" with line break
        NSString *baseString = [NSString stringWithFormat:@"%@ %@\n", comment.from.userName, comment.text];
        //make attributed string with username bold
